...
class VideoLoader(Executor):
    ...
    def __init__(self, max_num_frames: int = 50, fps=DEFAULT_FPS, **kwargs):
        """
    ...
    def extract(self, docs: DocumentArray, **kwargs):
        ...
        for doc in docs:
            ...
            if doc.uri == '':
                raise ValueError(f'No uri passed for the Document: {doc.id}')
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                frame_fn_list = self._convert_video_uri_to_frames(source_fn)
                ...
                for idx, frame_fn in enumerate(frame_fn_list):
                    ...
                    _chunk = Document(modality='image')
                    _chunk.blob = np.array(frame_fn).astype('uint8')
                    timestamp = idx
                ...
                audio, sample_rate = self._convert_video_uri_to_audio(source_fn)
                if audio is not None:
                    _chunk = Document(modality='audio')
                    _chunk.blob, _chunk.tags['sample_rate'] = audio, sample_rate
                    doc.chunks.append(_chunk)
    ...
    def _convert_video_uri_to_frames(self, source_fn):
        ...
        try:
            ...
            video_frames = np.frombuffer(out, np.uint8).reshape([-1, 960, 540, 3])
        except Exception as e:
            self.logger.error(
                f'Frame extraction failed for uri, {source_fn},\n{e.stderr}'
            )

        return video_frames

    def _convert_video_uri_to_audio(self, source_fn):
        data = None
        sample_rate = None
        try:
            out, _ = (
            ...
            self.logger.error(
                f'Audio extraction failed for uri, {source_fn},\n{e.stderr}'
            )
